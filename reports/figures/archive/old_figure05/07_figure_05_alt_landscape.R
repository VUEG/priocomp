...
all_nocosts <- get_stat_curves(zproject, variant_ids = c(8, 12, 19, 20)) %>%
  ...
  dplyr::mutate(variant = factor(variant,
                                 levels = c("08_abf_es", "04_abf_all_wgt_es", "20_load_bd_es",
                                            "12_abf_bd", "04_abf_all_wgt_bd", "19_load_es_bd"),
                                 ordered = TRUE))
# Costs
all_costs <- get_stat_curves(zproject, variant_ids = c(10, 14, 21, 22)) %>%
  dplyr::bind_rows(v06_costs, .) %>%
  dplyr::mutate(variant = factor(variant,
                                 levels = c("10_abf_es_cst", "06_abf_all_wgt_cst_es", "22_load_bd_es_cst",
                                            "14_abf_bd_cst", "06_abf_all_wgt_cst_bd", "21_load_es_bd_cst"),
                                 ordered = TRUE))

p1 <- all_nocosts %>%
  plot_curves(title = "", non_param = TRUE, invert_x = TRUE,
              nrow = 2, ncol = 3, highlights = highlights,
              plot_min = TRUE, plot_max = TRUE,
              labels = c("ES", "ES (rank ALL)", "ES (rank BD)",
                         "BD", "BD (rank ALL)", "BD (rank ES)"))
...
ggsave("reports/figures/figure05/01_figure_05_nocosts_alt_landscape.png", p1,
       width = 9, height = 6)
ggsave("reports/figures/figure05/02_figure_05_costs_alt_landscape.png", p2,
       width = 9, height = 6)
