...
blm = 0
target = 0.1
results_ms <- gurobi_solve(sp_rasters_filled, cost, targets = target, blm = blm)
...
milp_ms <- process_gurobi_results(results_ms, template)

milp_mc <- raster::stack(lapply(results_mc, process_gurobi_results, template))
names(milp_mc) <- paste0("b", budgets)

levelplot(milp_ms, main = "Minimum set solution\n(target = 10%, gap = 5%)",
          scales = list(draw = FALSE),
          col.regions = c("grey70", "#d7191c"),
          colorkey = list(space = "bottom", height = 1))
levelplot(milp_mc[[2]], main = "Maximum coverage solution\n(budget = 10%, gap = 5%)",
          scales = list(draw = FALSE),
          col.regions = c("grey70", "#d7191c"),
          colorkey = list(space = "bottom", height = 1))
...
(p9 <- ggplot(caz_zonal, aes(x = zone, y = drem, color = species)) + geom_line() + ggtitle("CAZ") +
  ylab("Distribution remaining\n") + xlab("\n Landscape selected") + 
  scale_x_continuous(breaks = 0:10, labels = paste0(seq(100, 0, -10), "%")))
(p10 <- ggplot(milp_zonal, aes(x = zone, y = drem, color = species)) + geom_line() + 
  ggtitle("Gurobi MCP") + ylab("Distribution remaining\n") + xlab("\n Landscape selected") + 
  scale_x_continuous(breaks = 0:10, labels = paste0(seq(100, 0, -10), "%")))
(p11 <- ggplot(all_zonal, aes(x = zone, y = mean_drem, color = method)) + geom_line() +
