...
```{r function-defintions,echo=FALSE}
...
While we will not be using cost data as in actually considering cost data, but we need for as a constraint data for the Gurobi MILP optimization. The cost is `1.0` in each cell, basically meaning that it translates into number of cells. This way it is possible to link an aerial target (such as the top 10% of the landscape) into a budgetary constraint. 
...
Here, we're testing three different prioritization methods against each other in increasing level of complexity:

1. Rarity-wighted richness
2. Zonation (two methods: core-area Zonation and additive benefit function)
3. Mixed integer programming (MILP) using Gurobi solver

### 2.1 Rarity-Weighted Richness (RWR)

From [Albuquerque and Beier (2009)](http://dx.plos.org/10.1371/journal.pone.0119905):
...
While the formulation by Albuquerque and Beier (2009) applies to binary presence/absence data, it can be extended to data where the values in each cell is a quantitative measure, e.g. the probability of occurrence. Hence, the formula becomes:
...
For a thorogh description of using [the Gurobi solver](http://www.gurobi.com/) for solving conservation planning problem, see [Beyer et al. (2016)](http://dx.doi.org/10.1016/j.ecolmodel.2016.02.005). Most of the approach and code below are based on the excellent tutorials [Integer Programming with Gurobi for Reserve Design](http://strimas.com/r/gurobi/) and [Field Guide to ILP Solvers in R for Conservation Prioritization](http://strimas.com/prioritization/ilp-field-guide/) by [Matt Strimas-Mackey](http://strimas.com/about/), and on the [protectr package](https://github.com/mstrimas/protectr) by the same person.

Below, the prioritizaton is done by solving the conservation planning problem in a form a *maximum coverage problem*, or in other words, find the set of planning units that maximizes the overall level of representation (as measured by the normalized occurrence levels) across a suite of conservation features, while keeping cost within a fixed budget. Here, the cost - and hence the budget - is defined to to be constant (1.0) in all cells. Thus, the budget corresponds to a particular number of planning units (cells) and we can maximize, for example, feature representation for the top 10% of the landscape. By repeating the maximum coverage optimization for a range of budgets (5%, 10%, 15%, ..., 100% of the landscape), it is possible to create a nested hierarchy of solutions. This hierarchical solutions resembles the rankings resulting from the RWR and Zonation above.
...
levelplot(subset(milp_mc, c(2, 4, 10)), main = "Maximum coverage solution\n(gap = 5%)",
          scales = list(draw = FALSE),
...
milp_mc_hier <- sum(milp_mc, na.rm = TRUE)
# Replace 0s with NAs
milp_mc_hier[milp_mc_hier == 0] <- NA
# Normalize value into scale [0, 1]
milp_mc_hier <- normalize(milp_mc_hier)
levelplot(milp_mc_hier, margin = FALSE, scales = list(draw = FALSE),
          par.settings = rasterTheme(region = z_colors_RdYlBu$colors), 
...
p4 <- levelplot(rwr_top10, main = "RWR\ntop 10%",
          ...
          col.regions = c("grey70", "#d7191c"),
          colorkey = list(space = "bottom", height = 1))
p5 <- levelplot(abf_rank_top10, main = "Zonation ABF\ntop 10%",
          scales = list(draw = FALSE),
          col.regions = c("grey70", "#d7191c"),
          colorkey = list(space = "bottom", height = 1))
p6 <- levelplot(milp_mc[[1]], main = "Gurobi MCP\n10% budget",
          scales = list(draw = FALSE),
          col.regions = c("grey70", "#d7191c"),
          colorkey = list(space = "bottom", height = 1))
...
cell_count <- lapply(c(rwr_top10, abf_rank_top10, milp_mc[[2]]), function(x) table(raster::getValues(x))) 
...
milp_zonal <- zonal_stats(milp_mc_hier, ol_sp_rasters, rcl, "MILP")

# Bind all zonal stats together
all_zonal <- dplyr::bind_rows(rwr_zonal, abf_zonal, caz_zonal, milp_zonal) %>% 
...
ggplot(caz_zonal, aes(x = zone, y = drem, color = species)) + geom_line() + ggtitle("CAZ") +
  ylab("Distribution remaining\n") + xlab("\n Landscape selected") + 
  scale_x_continuous(breaks = 0:10, labels = paste0(seq(100, 0, -10), "%"))
ggplot(milp_zonal, aes(x = zone, y = drem, color = species)) + geom_line() + 
