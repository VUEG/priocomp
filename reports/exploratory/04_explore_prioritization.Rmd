...
# Max coverage
b <- 0.1 * raster::cellStats(cost, "sum")
results_mc <- protectr::gurobi_maxcoverage(cost, sp_rasters_filled, budget = b)
...
milp_mc <- process_gurobi_results(results_mc, template)

levelplot(milp_ms, main = "Minimum set solution\n(target = 10%, gap = 5%)",
          scales = list(draw = FALSE),
          col.regions = c("grey70", "#d7191c"),
          colorkey = list(space = "bottom", height = 1))
levelplot(milp_mc, main = "Maximum coverage solution\n(budget = 10%, gap = 5%)",
          scales = list(draw = FALSE),
          col.regions = c("grey70", "#d7191c"),
          colorkey = list(space = "bottom", height = 1))
...
p5 <- levelplot(abf_rank_top10, main = "Zonation ABF\ntop 10%",
          ...
          col.regions = c("grey70", "#d7191c"),
          colorkey = list(space = "bottom", height = 1))
p6 <- levelplot(milp_mc, main = "Gurobi MCP\n10% budget",
          scales = list(draw = FALSE),
          col.regions = c("grey70", "#d7191c"),
          colorkey = list(space = "bottom", height = 1))
...
cell_count <- lapply(c(rwr_top10, abf_rank_top10, milp_mc), function(x) table(raster::getValues(x))) 
...
rcl <- data.frame(from = seq(0, .9, 0.1), 
                  to = seq(0.1, 1.0, 0.1), 
                  value = 1:10)
rcl_bin <- data.frame(from = c(0.0, 0.9), 
                  to = c(0.9, 1.0), 
                  value = 1:2)
...
milp_zonal <- zonal_stats(r_solution, ol_sp_rasters, rcl_bin, "MILP")
...
(p10 <- ggplot(all_zonal, aes(x = zone, y = mean_drem, color = method)) + geom_line() +
  ...
  scale_x_continuous(breaks = 0:10, labels = paste0(seq(100, 0, -10), "%")))
(p11 <- ggplot(milp_zonal, aes(x = zone, y = drem, color = species)) + geom_line() +
  ylab("Distribution remaining\n") + xlab("\n Landscape selected") )#+ 
  #scale_x_continuous(breaks = 0:10, labels = paste0(seq(100, 0, -10), "%")))
```
